...
class FheScoreParam:
    def __init__(
        ...
        # Select timing tables based on backend
        if use_gpu:
            self._mult_plain_table = mult_plain_time_gpu
            self._mult_table = mult_time_gpu
            self._rescale_table = rescale_time_gpu
            self._rotate_table = rotate_time_gpu
            self._add_table = add_time_gpu
        else:
            self._mult_plain_table = mult_plain_time
            self._mult_table = mult_time
            self._rescale_table = rescale_time
            self._rotate_table = rotate_time
            self._add_table = add_time

        # self.level = level
        if level > 0:
            self.mult_score = self._mult_table[self.input_degree][level]
            self.mult_plain_score = self._mult_plain_table[self.input_degree][level]
            self.rescale_score = self._rescale_table[self.input_degree][level]
        else:
            self.mult_score = 0
    ...
    def get_score(self) -> float:
        ...
        if op_counts and isinstance(next(iter(op_counts)), int):
            ...
            for lv, ops in op_counts.items():
                r_score = self._rotate_table[self.input_degree][lv]
                mp_score = self._mult_plain_table[self.input_degree][lv] if lv > 0 else 0
                m_score = self._mult_table[self.input_degree][lv] if lv > 0 else 0
                a_score = self._add_table[self.input_degree][lv]
                rs_score = self._rescale_table[self.input_degree][lv] if lv > 0 else 0
                score += (
                    ops['rotate'] * r_score
