...
# BlockColMajor mega layer imports (for custom task E2E tests)
...
from inference.model_generator.deploy_cmds import set_param
from inference.model_generator.layers.block_col_major_transpose import (
    BlockColMajorTranspose as BlockColMajorTransposeMega,
)
from inference.model_generator.layers.block_col_major_cpmm import BlockColMajorCPMM as BlockColMajorCPMMMega
from inference.model_generator.layers.block_col_major_ccmm import BlockColMajorCCMM as BlockColMajorCCMMMega
from inference.model_generator.layers.par_block_col_major_cpmm import ParBlockColMajorCPMM as ParBlockColMajorCPMMMega
from inference.model_generator.layers.par_block_col_major_transpose import (
    ParBlockColMajorTranspose as ParBlockColMajorTransposeMega,
)
from inference.model_generator.layers.par_block_col_major_ccmm import ParBlockColMajorCCMM as ParBlockColMajorCCMMMega
...
class TestE2ESingleLayer(CompilerTestBase):
    ...
    def _custom_task_deploy(self, test_name, input_args, output_args):
        """Deploy a custom task (non-compiler) for E2E testing.

        Generates mega_ag.json and task_signature.json in the task directory,
        and updates the ut_names.json manifest.

        IMPORTANT: Caller must call set_param() before building the compute graph
        (i.e. before call_custom_compute), since rotation operations require
        FHE parameters to be set.
        """
        output_dir = self.e2e_base_path / test_name
        server_dir = output_dir / 'task' / 'server'
        ...
        process_custom_task(
            ...
            output_instruction_path=str(server_dir),
            fpga_acc=False,
        )

        # Verify critical output files exist
        for required_file in ['task_signature.json', 'mega_ag.json']:
            assert (server_dir / required_file).exists(), f'Missing {required_file} in {server_dir}'
        ...
        print(f'  [E2E custom] {test_name} -> {server_dir}')

    def test_block_col_major_transpose(self):
        """BlockColMajorTranspose: 8x8 matrix, d=4, N=16384."""
        set_param('PN14QP438')
        M, K, D, LEVEL = 8, 8, 4, 5
        N_SLOT = 16384 // 2

        layer = BlockColMajorTransposeMega(shape=(M, K), block_size=D, n_slot=N_SLOT)
        n_cts = layer.num_blocks
        ...
        self._custom_task_deploy(
            'block_col_major_transpose',
            input_args=[
    ...
    def test_block_col_major_cpmm(self):
        """BlockColMajorCPMM: A(8x8) * B(8x8), d=4, N=16384."""
        set_param('PN14QP438')
        M, K, P, D, LEVEL = 8, 8, 8, 4, 5
        N_SLOT = 16384 // 2

        layer = BlockColMajorCPMMMega(shape_A=(M, K), shape_B=(K, P), block_size=D, n_slot=N_SLOT)
        n_A_cts = layer.num_block_rows_A * layer.num_block_cols_A
        ...
        output_cts = layer.call_custom_compute(A_cts, data_source)

        self._custom_task_deploy(
            'block_col_major_cpmm',
            input_args=[
    ...
    def test_block_col_major_ccmm(self):
        """BlockColMajorCCMM: A(8x8) * B(8x8), d=4, N=16384, level=7."""
        set_param('PN14QP438')
        M, N_dim, P, D, LEVEL = 8, 8, 8, 4, 7
        N_SLOT = 16384 // 2

        layer = BlockColMajorCCMMMega(shape_A=(M, N_dim), shape_B=(N_dim, P), block_size=D, n_slot=N_SLOT)

        A_cts = [
            CkksCiphertextNode(f'A_ct_{k}', level=LEVEL) for k in range(layer.num_block_rows_A * layer.num_block_cols_A)
        ]
        B_cts = [
            CkksCiphertextNode(f'B_ct_{k}', level=LEVEL) for k in range(layer.num_block_rows_B * layer.num_block_cols_B)
        ]
    ...
    def test_par_block_col_major_cpmm(self):
        ...
        layer = ParBlockColMajorCPMMMega(
    ...
    def test_par_block_col_major_transpose(self):
        ...
        layer = ParBlockColMajorTransposeMega(shape=(M, K), block_size=D, n_heads=N_HEADS, n_slot=N_SLOT)
    ...
    def test_par_block_col_major_ccmm(self):
        ...
        layer = ParBlockColMajorCCMMMega(
            shape_A=(M, N_dim),
            shape_B=(N_dim, P),
            block_size=D,
            n_heads=N_HEADS,
