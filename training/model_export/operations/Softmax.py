...
class SoftmaxComputeNode(ComputeNode):
    """Compute node for Softmax operation"""
    ...
    @staticmethod
    def from_onnx_node(x: NodeProto, features_nodes) -> 'SoftmaxComputeNode':
        ...
        return SoftmaxComputeNode(layer_id, layer_type, feature_input, feature_output)
    ...
    def to_torch_code(self) -> dict[str, list[str]]:
        log.debug('Generating softmax code')
        init_str, forward_str = [], []
        params_str = dict()
        params_str['num_features'] = self.feature_input[0].channel
        forward_str.append(f'{str(self.feature_output[0])} = F.softmax({str(self.feature_input[0])})')
