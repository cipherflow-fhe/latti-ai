...
class IdentityComputeNode(ComputeNode):
    def __init__(
        self, layer_id: str, layer_type: str, feature_input: list[FeatureNode], feature_output: list[FeatureNode]
    ):
        super(IdentityComputeNode, self).__init__(layer_id, layer_type, feature_input, feature_output)
        ...
        feature_output[0].dim = feature_input[0].dim
        feature_output[0].level = feature_input[0].level

    @staticmethod
    def from_onnx_node(x: NodeProto, features_nodes) -> 'IdentityComputeNode':
        ...
        return IdentityComputeNode(layer_id, layer_type, feature_input, feature_output)

    @override
    def to_torch_code(self) -> dict[str, list[str]]:
        log.debug('Generating identity code')
