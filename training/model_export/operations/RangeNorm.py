...
class RangeNormComputeNode(ComputeNode):
    """Compute node for RangeNorm operation"""

    def __init__(
        self, layer_id: str, layer_type: str, feature_input: list[FeatureNode], feature_output: list[FeatureNode]
    ):
        super(RangeNormComputeNode, self).__init__(layer_id, layer_type, feature_input, feature_output)
    ...
    @staticmethod
    def from_onnx_node(x: NodeProto, features_nodes) -> 'RangeNormComputeNode':
        ...
        return RangeNormComputeNode(layer_id, layer_type, feature_input, feature_output)
    ...
    def to_torch_code(self) -> dict[str, list[str]]:
        ...
        init_str.append(f'self.{self.layer_id} = RangeNormComputeNode({params})')
