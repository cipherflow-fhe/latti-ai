...
class DropoutComputeNode(ComputeNode):
    ...
    @staticmethod
    def from_onnx_node(x: NodeProto, features_nodes) -> 'DropoutComputeNode':
    ...
    def to_torch_code(self) -> dict[str, list[str]]:
        log.debug('Generating dropout code')
        init_str, forward_str = [], []
        params_str = {}
        params_str['p'] = self.ratio
        params = self.gen_params_str(**params_str)
        init_str.append(f'self.{self.layer_id} = nn.Dropout(**{{{params}}})')
