...
class SigmoidComputeNode(ComputeNode):
    """Compute node for Sigmoid operation"""
    ...
    def __init__(
        ...
        feature_output[0].level = 5

    @staticmethod
    def from_onnx_node(x: NodeProto, features_nodes) -> 'SigmoidComputeNode':
        ...
        return SigmoidComputeNode(layer_id, layer_type, feature_input, feature_output)
    ...
    def to_torch_code(self) -> dict[str, list[str]]:
        log.debug('Generating sigmoid code')
        init_str, forward_str = [], []
        params_str = dict()
        params_str['num_features'] = self.feature_input[0].channel
        forward_str.append(f'{str(self.feature_output[0])} = torch.sigmoid({str(self.feature_input[0])})')
