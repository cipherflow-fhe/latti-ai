...
class ConcatComputeNode(ComputeNode):
    """Compute node for Concat operation"""

    def __init__(
        self, layer_id: str, layer_type: str, feature_input: list[FeatureNode], feature_output: list[FeatureNode]
    ):
        super(ConcatComputeNode, self).__init__(layer_id, layer_type, feature_input, feature_output)
    ...
    def to_torch_code(self) -> dict[str, list[str]]:
        log.debug('Generating concat code')
        init_str, forward_str = [], []
        params_str = {}
        forward_str.append(
            f'{str(self.feature_output[0])} = torch.add({str(self.feature_input[0])}, {str(self.feature_input[1])})'
        )
