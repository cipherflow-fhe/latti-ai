...
class SplitComputeNode(ComputeNode):
    """Compute node for Split operation"""

    def __init__(
        self, layer_id: str, layer_type: str, feature_input: list[FeatureNode], feature_output: list[FeatureNode]
    ):
        super(SplitComputeNode, self).__init__(layer_id, layer_type, feature_input, feature_output)
        self.layer_type = 'split'
        feature_output[0].channel = feature_input[0].channel // 2
        ...
        feature_output[1].channel = feature_input[0].channel // 2
        feature_output[1].level = feature_input[0].level - 2
        feature_output[1].shape = feature_input[0].shape
        feature_output[1].skip = feature_output[0].skip
        feature_output[1].scale = feature_input[0].scale
        feature_output[1].skip = feature_input[0].skip

    @staticmethod
    def from_onnx_node(x: NodeProto, features_nodes) -> 'SplitComputeNode':
        ...
        return SplitComputeNode(layer_id, layer_type, feature_input, feature_output)

    @override
    def to_torch_code(self) -> dict[str, list[str]]:
        log.debug('Generating split code')
        init_str, forward_str = [], []
        params_str = {}
        params_str['num_features'] = self.feature_input[0].channel
        params_str['num_tensors'] = 2
        params = dict_to_args(params_str)
        init_str.append(f'self.{self.layer_id} = SplitComputeNode({params})')
