...
# FIXME: This algorithm needs to be improved to better than O(N^2)
...
def keep_following_outputs_discard_others(onnx_model: onnx.ModelProto, outputs_to_keep: Iterable[str]):
    ...
    assert set(output.name for output in graph_outputs) == outputs_to_keep_set
    ...
    for node_index, node in enumerate(onnx_model.graph.node):
        ...
        if node.op_type in op_types_to_remove:
            ...
            if non_constant_input is None:
                raise ValueError(
                    f'Trying to apply identity over a constant input.Node: {node.op_type}'
                )  # pragma: no cover
...
def clean_graph_at_node_op_type(onnx_model: onnx.ModelProto, node_op_type: str, fail_if_not_found: bool = True):
    ...
    if nodes_of_type:
        ...
        _clean_graph_at_node_name(onnx_model, node_name=node_to_cut_name, fail_if_not_found=fail_if_not_found)
...
def clean_graph_after_node_op_type(onnx_model: onnx.ModelProto, node_op_type: str, fail_if_not_found: bool = True):
...
def _clean_graph_at_node_name(onnx_model: onnx.ModelProto, node_name: str, fail_if_not_found: bool = True):
    ...
    initializers_name_to_instance = {initializer.name: initializer for initializer in onnx_model.graph.initializer}
...
# FIXME: Convert first Gather nodes
...
def convert_first_gather_to_matmul(
    ...
    for node in onnx_model.graph.node:
        if node.op_type == 'Gather' and node.input[1] in [input.name for input in onnx_model.graph.input]:
            ...
            matmul_node = onnx.helper.make_node('MatMul', inputs=[indices_input, data_input], outputs=[gather_output])
            onnx_model.graph.node.remove(node)
    ...
    pre_processing_graph = onnx.helper.make_graph(
        ...
        [
            onnx.helper.make_tensor_value_info(node.output[0], onnx.TensorProto.FLOAT, [None, depth])
            for node, depth in zip(pre_processing_nodes, gather_depths.values())
        ],
        depth_tensors + [values_tensor],
    )

    pre_processing_onnx = onnx.helper.make_model(pre_processing_graph, opset_imports=[onnx.helper.make_opsetid('', 14)])
