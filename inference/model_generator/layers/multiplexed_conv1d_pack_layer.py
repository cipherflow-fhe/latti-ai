...
class MultiplexedConv1DPackedLayer:
    ...
    def get_memory(self, bytes_per_plaintext: int = 0) -> dict[str, int]:
        ...
        n_packed_out = _math.ceil(self.n_out_channel / self.n_channel_per_ct)
        needs_rearrange = self.skip > 1 or self.stride > 1
        n_select = min(self.n_block_per_ct, self.n_out_channel) if needs_rearrange else 0
        counts = {
            'weight': n_out_groups * self.n_packed_in_channel * self.n_block_per_ct * self.kernel_shape,
            'bias': n_packed_out,
            'mask': n_select,
        }
