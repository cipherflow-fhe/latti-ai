...
void InitInferenceProcess::init_conv1d_layer(const string& key, const json& layer, const hid_t& h5_file) {
    ...
    if (style == "multiplexed") {
        ...
        ckks_conv1ds[key] = move(conv_layer);
    }
}
            ...
            if (groups == 1) {
                if (is_big_size) {
                    data_sources.emplace_back(
                        key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_big_conv2ds.at(key).get())));
                } else if (fp->pack_style == "multiplexed") {
                    data_sources.emplace_back(
                        key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_multiplexed_conv2ds.at(key).get())));
                } else {
                    data_sources.emplace_back(
                        key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_conv2ds.at(key).get())));
                }
            } else {
                if (is_big_size) {
                    data_sources.emplace_back(
                        key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_big_dw_conv2ds.at(key).get())));
                } else if (fp->pack_style == "multiplexed") {
                    data_sources.emplace_back(key, fhe_ops_lib::CustomData(static_cast<void*>(
                                                       fp->ckks_multiplexed_dw_conv2ds.at(key).get())));
                } else {
                    data_sources.emplace_back(
                        key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_dw_conv2ds.at(key).get())));
                }
            }
        } else if (layer_type == "fc0" || layer_type == "fc1") {
            data_sources.emplace_back(key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_denses.at(key).get())));
        } else if (layer_type == "poly_relu2d" || layer_type == "polyact") {
            auto feature_input = layer.value()["feature_input"].get<vector<string>>();
            FeatureNode d_input_node(json_features[feature_input[0]]);
            if (d_input_node.dim == 0) {
                data_sources.emplace_back(
                    key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_poly_relu_0d.at(key).get())));
            } else if (d_input_node.dim == 1) {
                data_sources.emplace_back(
                    key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_poly_relu_1d.at(key).get())));
            } else {
                data_sources.emplace_back(
                    key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_poly_relu.at(key).get())));
            }
        } else if (layer_type == "upsample_nearest") {
            data_sources.emplace_back(
                key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_upsample_nearest.at(key).get())));
        } else if (layer_type == "conv1d") {
            string style = layer.value().value("style", string("ordinary"));
            if (style == "multiplexed") {
                int groups = layer.value().value("groups", 1);
                ...
                if (groups == n_out_channel && groups != 1) {
                    data_sources.emplace_back(key, fhe_ops_lib::CustomData(static_cast<void*>(
                                                       fp->ckks_multiplexed_dw_conv1ds.at(key).get())));
                } else {
                    data_sources.emplace_back(
                        key, fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_multiplexed_conv1ds.at(key).get())));
                }
            } else {
                data_sources.emplace_back(key,
                                          fhe_ops_lib::CustomData(static_cast<void*>(fp->ckks_conv1ds.at(key).get())));
            }
        }
