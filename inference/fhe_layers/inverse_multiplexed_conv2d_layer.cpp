...
InverseMultiplexedConv2DLayer::InverseMultiplexedConv2DLayer(const CkksParameter& param_in,
    ...
    std::array<uint64_t, 4UL> weight_shape = weight_.get_shape();
    if (padding_in.get(0) < 0 && padding_in.get(1) < 0) {
        pad_ = to_int((kernel_shape_ - Duo{1, 1}) / 2);
...
void InverseMultiplexedConv2DLayer::prepare_weight() {
    ...
    weight_pt.clear();
    bias_pt.clear();
    weight_pt.resize(n_out_channel_);
    for (int i = 0; i < (int)weight_pt.size(); i++) {
        weight_pt[i].resize(n_in_channel_);
    }
    bias_pt.resize(n_out_channel_);

    CkksContext ctx = CkksContext::create_empty_context(this->param_);

    int total_kernel_count = kernel_shape_[0] * kernel_shape_[1] * output_step[0] * output_step[1];
    parallel_for(n_out_channel_, th_nums, ctx, [&](CkksContext& ctx_copy, int out_channel_idx) {
        for (int in_channel_idx = 0; in_channel_idx < n_in_channel_; ++in_channel_idx) {
...
CkksPlaintextRingt InverseMultiplexedConv2DLayer::generate_weight_pt_for_indices(CkksContext& ctx,
    ...
    double w_val = weight_.get(out_channel_idx, in_channel_idx, kernel_idx[0], kernel_idx[1]);
...
CkksPlaintextRingt InverseMultiplexedConv2DLayer::generate_bias_pt_for_index(CkksContext& ctx,
    ...
    for (int linear_idx = 0; linear_idx < cached_total_block_size; ++linear_idx) {
        bias_vec[linear_idx] = bias_.get(out_channel_idx);
    }
...
std::vector<uint32_t> InverseMultiplexedConv2DLayer::get_used_input_indices() const {
    ...
    for (uint32_t n_in_ch = 0; n_in_ch < n_in_channel_; n_in_ch++) {
        uint32_t base = n_in_ch * prod(ct_stride_) * prod(output_step);
...
vector<CkksCiphertext> InverseMultiplexedConv2DLayer::run_core(CkksContext& ctx, const std::vector<CkksCiphertext>& x) {
    std::vector<std::vector<lattisense::CkksCiphertext>> rotated_x(n_in_channel_);

    parallel_for(n_in_channel_, th_nums, ctx, [&](CkksContext& ctx_copy, int in_channel_idx) {
        uint32_t base_in_ct_idx = in_channel_idx * prod(ct_stride_) * prod(output_step);
        for (const Duo& r2_pos : duo_range(output_step)) {
            for (const Duo& seg_pos : duo_range(ct_stride_)) {
                if (seg_pos[0] >= kernel_shape_[0] || seg_pos[1] >= kernel_shape_[1])
                    continue;
    ...
    uint32_t n_weight = weight_pt.empty() ? n_out_channel_ : weight_pt.size();
    ...
    parallel_for(n_weight, th_nums, ctx, [&](CkksContext& ctx_copy, int ct_idx) {
        for (const Duo& r2_pos : duo_range(output_step)) {
            ...
            int base_idx = r2_linear * prod(kernel_shape_);
            uint32_t n_j = weight_pt.empty() ? n_in_channel_ : weight_pt[ct_idx].size();
            for (int j = 0; j < n_j; j++) {
                for (int k = 0; k < kernel_shape_[0] * kernel_shape_[1]; k++) {
                    if (weight_pt.empty()) {
    ...
    if (need_repack) {
        ...
        uint32_t n_out_ct = div_ceil(n_out_channel_, n_channel_per_ct_repack);

        // Step 1: mask all channels with the shared repack mask
        parallel_for(n_out_channel_, th_nums, ctx, [&](CkksContext& ctx_copy, int c) {
