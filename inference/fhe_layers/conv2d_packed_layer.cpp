...
void Conv2DPackedLayer::prepare_weight() {
    ...
    parallel_for(n_packed_ct_out_, th_nums, ctx, [&](CkksContext& ctx_copy, int packed_out_ct_idx) {
        ...
        for (uint32_t j = 0; j < n_packed_ct_in_ * n_channel_per_ct_; j++) {
            weight_pt_[packed_out_ct_idx][j].resize(kernel_size);
            for (uint32_t k = 0; k < kernel_size; k++) {
                weight_pt_[packed_out_ct_idx][j][k] = generate_weight_pt_for_indices(ctx_copy, packed_out_ct_idx, j, k);
            }
        }
    });

    bias_pt_.resize(n_packed_ct_out_);
    parallel_for(n_packed_ct_out_, th_nums, ctx, [&](CkksContext& ctx_copy, int packed_out_ct_idx) {
