...
ls::CkksCiphertext SoftmaxLayer::broadcast_slots(ls::CkksContext& ctx, ls::CkksCiphertext ct, uint32_t skip) {
...
SoftmaxLayer::SoftmaxLayer(const ls::CkksParameter& param,
...
ls::CkksCiphertext SoftmaxLayer::sum_slots(ls::CkksContext& ctx, ls::CkksCiphertext ct, uint32_t skip) {
    ls::CkksCiphertext res = std::move(ct);
    for (uint32_t step = 1; step < n_channel_per_ct_; step <<= 1) {
        uint32_t physical_shift = step * skip;
...
Feature0DEncrypted SoftmaxLayer::run(ls::CkksContext& ctx, const Feature0DEncrypted& x) {
    ...
    auto input_level = x.data[0].get_level();
    printf("[LEVEL] input_level: %d \n", input_level);
    ...
    sum_feat = sum_feat.refresh_ciphertext();
    ...
    // inv后再次自举刷新level
    printf("[LEVEL] before bts_2: %d\n", inv_sum.get_level());
    Feature0DEncrypted inv_feat(&ctx, inv_sum.get_level());
    inv_feat.n_channel = 1;
    inv_feat.n_channel_per_ct = 1;
    inv_feat.skip = 1;
    inv_feat.pack_type = exp_x.pack_type;
    inv_feat.data.push_back(std::move(inv_sum));
    inv_feat = inv_feat.refresh_ciphertext();
    inv_feat.level = inv_feat.data[0].get_level();
    inv_sum = std::move(inv_feat.data[0]);
    printf("[LEVEL] after bts_2: %d\n", inv_sum.get_level());
