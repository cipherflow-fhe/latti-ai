...
public:
    SoftmaxLayer(const ls::CkksParameter& param,
                 ...
                 int exp_order = 0,
                 int inv_order = 0,
                 int ciphertext_skip = 1, int total_n_channel = 4, double input_min = 2.0, double input_max = 0.0);
...
private:
    ...
    // 槽位旋转累加
    // ls::CkksCiphertext sum_slots(ls::CkksContext& ctx, ls::CkksCiphertext ct);
    ls::CkksCiphertext sum_slots(ls::CkksContext& ctx, ls::CkksCiphertext ct, uint32_t skip);
    // 内部辅助：将首槽结果广播至所有有效通道
    // ls::CkksCiphertext broadcast_slots(ls::CkksContext& ctx, ls::CkksCiphertext ct);
