...
- Client process handles `enc_to_share_simple` with `decrypt_to_share_simple`: receive encrypted masked feature, decrypt, and store each floating-point share in `Feature2DShare::data_double`.
- Client process handles `share_to_enc_simple` with `encrypt_from_share_simple`: encrypt values from `Feature2DShare::data_double`.
- Server helper: `server_enc_to_share_simple(...)`.
- Server helper: `server_share_to_enc_simple(...)`.
- Data struct methods: `Feature2DEncrypted::split_to_shares_simple(...)`, `decrypt_to_share_simple(...)`, `encrypt_from_share_simple(...)`, and `combine_with_share_simple(...)`.
