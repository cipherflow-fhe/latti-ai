...
// ──────────────────────────────────────────────────────────────────────────────
class SoftmaxTestFixture : public LattigoCkksBtpToyFixture  {
public:
    SoftmaxTestFixture(
    ...
    // 供普通 TEST_CASE 访问
    ls::CkksBtpContext& get_context() { return context; }
...
// ──────────────────────────────────────────────────────────────────────────────
TEST_CASE_METHOD(SoftmaxTestFixture, "Softmax basic functionality", "[softmax]") {
    printf("[DEBUG] max_level=%d\n", max_level);
    for (int l = 0; l <= max_level; l++) {
        printf("level %d: q_bits = %d\n", l, (int)log2(get_param().get_q(l)));
    }
    const uint32_t n_channel_per_ct = 4;
    const int level_in = 9;   // 足够容纳深度

    SoftmaxLayer layer(context.get_parameter(), *exp_coeffs, *inv_coeffs, n_channel_per_ct, 
                        level_in, exp_order, inv_order, 1, 4, -2.0, 0.0);

    // 随机生成 4 个在 [-4.0, 0.0] 内的输入
    ...
    // ── 计时：仅包裹 layer.run ─────────────────────────────────────────
    Feature0DEncrypted result(nullptr, 0); 
    {
        ScopedTimer t("base softmax - layer.run");
        result = layer.run(context, encrypted_input);
    }
    ...
    // 统计误差
    auto got_arr = SoftmaxTestFixture::make_array(decrypted);
    auto exp_arr = SoftmaxTestFixture::make_array(expected);
...
TEST_CASE("Softmax after ReLU simulation", "[softmax]") {
    ...
    const int level_in = 9;
    // 输入范围 [0.0, 4.0]，4 通道，exp 用 7 阶 Chebyshev，倒数用 4 阶
    SoftmaxTestFixture fix(0.0, 4.0, total_inputs, 7, 4);
    // 2. 构建 SoftmaxLayer，传入 Chebyshev 系数等参数
    SoftmaxLayer layer(fix.get_param(), *fix.exp_coeffs, *fix.inv_coeffs, n_channel_per_ct,
    ...
    std::uniform_real_distribution<double> dist(0.0, 4.0); // 设定实际输入范围，注意要与上面的一致（3个位置）
    ...
    Feature0DEncrypted result(nullptr, 0);
    {
        ScopedTimer t("ReLU softmax – layer.run");
        result = layer.run(fix.get_context(), encrypted);
    }
...
TEST_CASE("Print BTP parameter spec", "[btp_spec]") {
    auto btp_param = ls::CkksBtpParameter::create_toy_parameter();
    auto ctx = ls::CkksBtpContext::create_random_context(btp_param);
    auto& param = ctx.get_parameter();
    
    printf("=== Client BTP Parameter ===\n");
    printf("N (poly_degree * 2): %llu\n", (unsigned long long)param.get_n() * 2);
    printf("n_slots (N/2):       %llu\n", (unsigned long long)param.get_n());
    printf("max_level:           %d\n",   param.get_max_level());
    printf("log_default_scale:   %d\n",   (int)std::round(std::log2(param.get_default_scale())));
    
    // 用 fixture 的 context 成员直接对比，不调用 get_param()
    SoftmaxTestFixture fix;   // 替换这一行
    auto& fix_param = fix.get_param();   // SoftmaxTestFixture 有这个方法
    
    printf("\n=== Fixture BTP Parameter ===\n");
    printf("N (poly_degree * 2): %llu\n", (unsigned long long)fix_param.get_n() * 2);
    printf("max_level:           %d\n",   fix_param.get_max_level());
    printf("log_default_scale:   %d\n",   (int)std::round(std::log2(fix_param.get_default_scale())));
    // printf("param_name: %s\n", fix_param.get_name().c_str());
    REQUIRE(param.get_n() == fix_param.get_n());
    REQUIRE(param.get_max_level() == fix_param.get_max_level());
}