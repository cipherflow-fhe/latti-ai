...
// Example: Encrypted inference on CIFAR-10 using CKKS with bootstrapping.
//
// This demo loads a pre-trained ResNet-20 model for CIFAR-10 and runs
// inference on a single encrypted image (3x32x32).  It then compares
// the ciphertext result against a plaintext reference to verify correctness.
...
int main(int argc, char* argv[]) {
    ...
    cout << "========== CIFAR-10 Encrypted Inference ==========" << endl;
    ...
    // ==================== 2. Server Side: Load Model ====================
    // Load the pre-compiled encrypted computation graph and model weights.
    // Bootstrapping (BTP) parameters are loaded since init_parameters(true).
    InitInferenceProcess init("./task/server/", false);
    init.init_parameters(true);
    init.is_lazy = false;
    init.load_model_prepare();

    // ==================== 3. Client Side: Encrypt Input ====================
    ...
    // Create CKKS encryption context with bootstrapping support.
    ...
    Feature2DEncrypted input_ct(&context, level, {1, 1});

    if (pack_style == "ordinary") {
        input_ct.pack(input_array, false, param.get_ckks_parameter().get_default_scale());
        ...
        input_ct.split_with_stride_pack(input_array, block_shape, channel_packing_factor, false,
                                        param.get_ckks_parameter().get_default_scale());
    } else {
        // Image fits in one ciphertext: use parallel multiplexed packing
        // to encode multiple channels into a single ciphertext.
        input_ct.par_mult_pack(input_array, false, param.get_ckks_parameter().get_default_scale());
    }
    ...
    context_map["param0"] = make_unique<CkksBtpContext>(move(context.shallow_copy_context()));
