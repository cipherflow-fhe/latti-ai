...
def convert_to_poly_model(model, args):
    ...
    log.info(
        'Poly convert: MaxPool2d=%d, ReLU=%d, poly=%s, upper_bound=%s, degree=%s',
        n_maxpool,
        n_relu,
        poly_cls.__name__,
        args.upper_bound,
...
def export_model(model, args):
    ...
    from training.nn_tools.replace import fuse_linear_bn1d_inplace
    ...
    fuse_linear_bn1d_inplace(export_model)
...
def initialize_lazy_buffers(model, input_shape):
...
def checkpoint_score(path):
    ...
    return metric_score(ckpt.get('metrics'))


def resolve_resume_checkpoint(resume, output_dir):
    if resume == 'last':
        return os.path.join(output_dir, 'last.pth')
    if resume == 'best':
        return os.path.join(output_dir, 'best.pth')
    return resume
...
def parse_args():
    ...
    parser.add_argument('--pretrained', default=None, help='path to checkpoint for fine-tuning')
    parser.add_argument('--resume', default=None, help='resume checkpoint: last, best, or a checkpoint path')
    parser.add_argument('--start-epoch', type=int, default=None, help='override first epoch number when resuming')
    ...
    parser.add_argument('--export-checkpoint', default='best', help='checkpoint to export: best, last, current, or a checkpoint path')
...
def main():
    ...
    if args.resume:
        ...
        resume_epoch, resume_metrics = load_resume_checkpoint(model, optimizer, resume_path, device)
        start_epoch = args.start_epoch or resume_epoch + 1
        best_score = checkpoint_score(best_path) or metric_score(resume_metrics)
        log.info('Resume checkpoint=%s start_epoch=%d end_epoch=%d', resume_path, start_epoch, args.epochs)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        save_checkpoint(model, optimizer, epoch, metrics, last_path)
    ...
    if args.export:
        if args.export_checkpoint != 'current':
            export_checkpoint = resolve_resume_checkpoint(args.export_checkpoint, args.output_dir)
            load_pretrained(model, export_checkpoint)
            log.info('Export checkpoint: %s', export_checkpoint)
        else:
            log.info('Export checkpoint: current model')
        model = model.to(device)
