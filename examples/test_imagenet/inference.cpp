...
// Example: Encrypted inference on ImageNet using CKKS with bootstrapping.
//
// This demo loads a pre-trained ImageNet model and runs inference on a
// single encrypted image (3x256x256).  The image is split into 128x128
// blocks before encryption.  It then compares the ciphertext result
// against a plaintext reference to verify correctness.
...
int main(int argc, char* argv[]) {
    ...
    cout << "========== ImageNet Encrypted Inference ==========" << endl;
    ...
    if (pack_style == "ordinary") {
        input_ct.pack(input_array, false, context.get_parameter().get_default_scale());
        ...
        input_ct.par_mult_pack(input_array, false, context.get_parameter().get_default_scale());
    }
