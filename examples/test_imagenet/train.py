...
"""Train MobileNetV2 on ImageNet (baseline or poly-ReLU).

Baseline (standard ReLU6):
  python train.py --data-dir /path/to/imagenet --epochs 90

Poly-ReLU (replace ReLU6 with RangeNormPoly2d, fine-tune, export):
  python train.py --data-dir /path/to/imagenet --poly_model_convert --pretrained train_baseline.pth --epochs 10

The --poly_model_convert flag enables ReLU6 -> RangeNormPoly2d replacement and
...
def main():
    ...
    # Poly-ReLU options
    parser.add_argument('--poly_model_convert', action='store_true', help='replace ReLU6 with RangeNormPoly2d')
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help='C H W input shape for ONNX export, e.g. --input-shape 3 256 256',
    )
    args = parser.parse_args()

    if args.poly_model_convert and args.input_shape is None:
        parser.error('--input-shape is required when --poly_model_convert is enabled, e.g. --input-shape 3 256 256')
    ...
    # Optionally replace ReLU6 -> RangeNormPoly2d
    if args.poly_model_convert:
        ...
        log.info(f'Device: {device}  |  ReLU6 {n_relu6} -> Poly {n_poly} (ub={args.upper_bound}, deg={args.degree})')
    else:
